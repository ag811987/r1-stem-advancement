...
sync=datetime.datetime.now().strftime("%b %d, %Y")
...
.b-mem{background:#e6f6ec;color:#1a7a44}.b-sup{background:#e5f0ff;color:#1a56c4}.b-oth{background:#f3eafe;color:#6b3fbf}
...
.ff{font-size:11px;color:#6b7280;max-width:280px}
...
<p class="sub">Program officers, science-program directors, and grants leadership at private research funders. Sources: Science Philanthropy Alliance member &amp; supporter roster, plus additional funders from the UMass CFR &ldquo;Private Foundations that Fund Research&rdquo; guide. From official foundation team/staff pages. Compiled ''' + sync + r'''. Emails marked <span class="gtag">guess</span> were inferred from a confirmed pattern and are unverified.</p>
<div class="stats">
...
<div class="stat"><div class="n" id="s-mem">0</div><div class="l">SPA Members</div></div>
<div class="stat"><div class="n" id="s-sup">0</div><div class="l">SPA Supporters</div></div>
<div class="stat"><div class="n" id="s-oth">0</div><div class="l">Other (UMass)</div></div>
...
<select id="fEmail" onchange="render()"><option value="">Any email</option><option value="confirmed">Confirmed only</option></select>
...
<th onclick="sortBy('program')">Program</th><th onclick="sortBy('focus')">Focus</th>
<th onclick="sortBy('email')">Email</th><th onclick="sortBy('phone')">Phone</th>
</tr></thead><tbody id="rows"></tbody></table>
<p class="foot">Donor-facing grantmaking staff only (program officers/directors, science/scientific leads, grants management, and leaders who drive grantmaking); communications, HR, IT, finance, and board-only members excluded. Several small/family foundations publish no staff directory and appear as a single row. Emails/phones are as published; blank where a funder does not publish them.</p>
</div>
<script>
const DATA=__DATA__;
let sortKey="funder",sortDir=1;
const badge=c=>({"SPA Member":"b-mem","SPA Supporter":"b-sup","Other (UMass)":"b-oth"}[c]||"b-oth");
...
function filt(){const q=document.getElementById('q').value.toLowerCase(),ff=document.getElementById('fFunder').value,fc=document.getElementById('fCat').value,fe=document.getElementById('fEmail').value;
return DATA.filter(d=>{if(ff&&d.funder!==ff)return false;if(fc&&d.category!==fc)return false;if(fe&&d.email_status!==fe)return false;if(q){const b=(d.funder+d.name+d.title+d.program+d.focus+d.email+d.ffocus).toLowerCase();if(!b.includes(q))return false}return true})}
...
tr.innerHTML=`<td><b>${d.funder}</b>${web}<br><span class="ff">${d.ffocus||''}</span></td><td><span class="badge ${badge(d.category)}">${d.category}</span></td><td>${d.name}</td><td>${d.title}</td><td>${d.program||'<span class=muted>—</span>'}</td><td>${d.focus||'<span class=muted>—</span>'}</td><td>${em}</td><td>${d.phone?d.phone:'<span class=muted>—</span>'}</td>`;tb.appendChild(tr)});
document.getElementById('count').textContent=`${rows.length} of ${DATA.length} contacts shown`;
document.getElementById('s-f').textContent=new Set(DATA.map(d=>d.funder)).size;
document.getElementById('s-p').textContent=DATA.filter(d=>d.name!=='—').length;
document.getElementById('s-mem').textContent=new Set(DATA.filter(d=>d.category==='SPA Member').map(d=>d.funder)).size;
document.getElementById('s-sup').textContent=new Set(DATA.filter(d=>d.category==='SPA Supporter').map(d=>d.funder)).size;
document.getElementById('s-oth').textContent=new Set(DATA.filter(d=>d.category==='Other (UMass)').map(d=>d.funder)).size;
document.getElementById('s-conf').textContent=DATA.filter(d=>d.email_status==='confirmed').length}
function cols(d){return[d.funder,d.category,d.name,d.title,d.program,d.focus,d.email,d.email_status,d.phone,d.website,d.ffocus,d.src]}
const HEAD=["Funder","Category","Name","Title","Program","Focus","Email","Email status","Phone","Website","What they fund","Source"];
...
open("funders.html","w").write(html.replace("__DATA__",DATA))
with open("research_funders_contacts.csv","w",newline="") as f:
    w=csv.writer(f);w.writerow(["funder","category","name","title","program","focus","email","email_status","phone","website","what_they_fund","source"])
    for r in sorted(rows,key=lambda x:(x["category"],x["funder"],x["name"])):
        w.writerow([r["funder"],r["category"],r["name"],r["title"],r["program"],r["focus"],r["email"],r["email_status"],r["phone"],r["website"],r["ffocus"],r["src"]])
